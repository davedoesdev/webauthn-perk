...
1.  Run [test/example.js](test/example.js) on your server
...
-   `/cred/*id*/` for each `*id*` in
    ...
    -   GET requests:
        ...
        -   If no public key is associated with `*id*` then the response
            ...
            [`CredentialCreation`](http://rawgit.davedoesdev.com/davedoesdev/webauthn4js/master/docs/types/CredentialCreation.html)
            and an encrypted
            [`SessionData`](http://rawgit.davedoesdev.com/davedoesdev/webauthn4js/master/docs/types/SessionData.html)
            returned by
        ...
        -   If a public key has been associated with `*id*` then the
            ...
            [`CredentialAssertion`](http://rawgit.davedoesdev.com/davedoesdev/webauthn4js/master/docs/types/CredentialAssertion.html)
            and an encrypted
            [`SessionData`](http://rawgit.davedoesdev.com/davedoesdev/webauthn4js/master/docs/types/SessionData.html)
            returned by
    ...
    -   PUT requests:
        ...
        -   The request body should contain a
            [`CredentialCreationResponse`](http://rawgit.davedoesdev.com/davedoesdev/webauthn4js/master/docs/types/CredentialCreationResponse.html)
    ...
    -   POST requests:
        ...
        -   The request body should contain a
            [`CredentialAssertionResponse`](http://rawgit.davedoesdev.com/davedoesdev/webauthn4js/master/docs/types/CredentialAssertionResponse.html)
...
`<script type="module">`. It exports the `PerkWorkflow` class.
...
-   Called before the browser’s Web Authentication API is invoked to
    ...
    You might display a prompt to ask the user to register their token,
    for example.

async after_register()  
-   Called after the Web Authentication API has generated a signature
    using the user’s security token.
...
async before_verify()  
-   Called when a security token has already been registered against the
    credential ID.

-   Called before the browser’s Web Authentication API is invoked to
    sign the verification challenge received from the server.

    You might display a prompt to ask the user to verify their token,
    for example.
...
-   Called after the signature is sent to the server in order to verify
    the user’s token is the same as the one registered against the
    credential ID.
...
-   The implementation in `PerkWorkflow` calls the Web Authentication
    API to sign a verification challenge received from the server and
    then sends the signature back to the server.
    ...
    Although you won’t know whether the user’s token is the same as the
    one registered against the credential ID, if it isn’t then your
    server will not successfully verify URLs returned by
    [perk(jwt)](#perk).
